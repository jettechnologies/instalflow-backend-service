...
export const requireAuth = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      throw new AppError(401, "Missing or invalid authorization header", ErrorType.UNAUTHORIZED);
    }
    ...
    req.user = decoded; // Cache safely for downstream action execution
    next();
  } catch (err) {
    next(new AppError(401, "Token expired or invalid constraints", ErrorType.UNAUTHORIZED));
  }
};
...
export const requireRole = (roles: string[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    if (!req.user || !roles.includes(req.user.role)) {
      return next(new AppError(403, "Forbidden: Insufficient privileges for action", ErrorType.FORBIDDEN));
    }
    next();
  };
};
