...
**Publishes**: `DomainEvent.INSTALLMENT_PAID`. (Contract lifecycle transitions — activation, `CONTRACT_WRITTEN_OFF`, `CONTRACT_RESTRUCTURED` — exist as `InternalNotificationType` values today but are **not yet mirrored as `DomainEvent`s**; see Extension points.)
...
**Scope guard, still true**: nothing in the codebase gates API access by subscription status — `RESTRICTED` is a tracked state, not an enforcement point. Building actual access restriction is a separate, deliberate product decision, not a side effect of this work.
...
- `DomainEvent.ORDER_CREATED` / `ORDER_CANCELLED` / `ORDER_STATUS_UPDATED` exist in `event.types.ts` and have handlers in `notification.handler.ts`, but no `Order` model exists in `schema.prisma`. Either dead code from an earlier design or a forward-looking stub — confirm before extending it.
- Contract lifecycle transitions (`CONTRACT_WRITTEN_OFF`, `CONTRACT_RESTRUCTURED`) exist as `InternalNotificationType` enum values (implying in-app notifications fire) but have no corresponding `DomainEvent` — meaning they may not trigger email/hub dispatch the way installment reminders do. Worth confirming this is intentional.
- Commission accrual has an `InternalNotificationType.COMMISSION_ACCRUED` value but no matching `DomainEvent` — accrual notifications appear to go through the internal (DB-only) path exclusively, not email. Confirm this is by design before assuming all "core" events reach every channel.
- §3's and §10's new `DomainEvent`s (11 total: 5 settlement, 6 subscription) have no `onEvent()` handler in `notification.handler.ts` (the local in-app/direct-email path) — see §3's "Notifications" note and §4's "Not subscribed locally." They *are* fully wired end-to-end via the notification-hub: all 11 have entries in `workers/notification-hub/src/router/event.router.ts` and matching branded HTML files in `mail-templates/`. Confirm this dual-path split (local handler vs. hub) is intentional before assuming every future event needs both.
- Three Prisma migrations are pending against the dev database as of this build (marketer-optional checkout, merchant settlement models, subscription grace period) — run `pnpm run db:migrate --name <name>` for each schema change in `prisma/schema.prisma`'s history before deploying; migrations could not be applied from the environment this work was done in (no network path to the Neon host).
