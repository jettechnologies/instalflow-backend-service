...
**Reminder cadence** (produced by `job-workers/installment-payment-reminder.worker.ts` → `PaymentReminderWorker`, cron-driven by `schedulers/installment-payment-reminder.scheduler.ts` daily at 00:00): 3-day-before, 1-day-before, due-today, recurring-overdue, 3-day-overdue (escalates to marketer), 7-day-overdue (escalates to marketer **and** admin). This escalation ladder is a core product deliverable, not a generic utility — treat changes to it with the same care as ledger changes. The `dayWindow()`/`ensureReminderSent()` idempotency helpers were extracted to `src/shared/utils/helpers/date-window.ts` (used by both this worker and §10's subscription renewal worker) — no behavior change, pure de-duplication.

**Shipped**: per-company reminder configuration. `CompanyReminderSettings` (one row per company that has customized anything; absent = hardcoded defaults) lets a company enable/disable each of the six reminder types and, for four of them (3-day-before, 1-day-before, overdue→marketer, overdue→admin), pick the offset from a fixed `ReminderOffsetOption` enum (`TWO_DAYS`/`THREE_DAYS`/`SEVEN_DAYS` — deliberately not a free integer range) via `ReminderSettingsService` (`getForCompany`/`getEffectiveSettingsMap`/`updateForCompany`, the last enforcing `1DayOffset < 3DayOffset` and `overdue3DayOffset ≤ overdue7DayOffset`). COMPANY-only `GET`/`PATCH /company/reminder-settings`. `PaymentReminderWorker` now loads one settings map per scan and widens its before-due/overdue-escalation queries to the full `[today, today+7d]`/`[today-7d, today]` range, filtering per-installment against the installment's company's effective offset — the DB query is no longer offset-specific, the in-memory comparison is. Field names keep their original `3Day`/`1Day`/`Overdue3Day`/`Overdue7Day` suffixes (matching the still-unrenamed `DomainEvent`s) even though `reminder1DayOffset` now defaults to `TWO_DAYS` — a called-out trade-off, not a bug. Two things fixed incidentally while touching this file: (1) a pre-existing idempotency bug where `ensureReminderSent()`'s result was checked but never gated on, so `emitEvent` fired on every daily scan regardless of the Redis guard — now correctly `continue`s when already sent, matching the pattern `SubscriptionRenewalWorker` already used correctly; (2) hardcoded day-count text ("due in 3 days", "Due Tomorrow", "7-Day Overdue Escalation") in both the hub router's subject lines and the six affected `mail-templates/*.html` files, now driven by the payload's `daysUntil`/`daysOverdue` — necessary for the config to mean anything, since a company that configures `SEVEN_DAYS` previously would have gotten an email still claiming "3 days".

**Extension points**: SMS/WhatsApp channel (no Twilio or equivalent is wired in today despite being named in `PROJECT_WORKFLOW.md`'s tech stack — that line is aspirational, not implemented; `CompanyReminderSettings` only controls cadence/enablement, not channel), outbound webhooks so a merchant's own system can react to these same events (currently these events only reach InstalFlow's own channels).
...
- Five Prisma migrations are pending against the dev database as of this build (marketer-optional checkout, merchant settlement models, subscription grace period, `CompanySubscription` status index, `CompanyReminderSettings`) — run `pnpm run db:migrate:deploy` before deploying; migrations could not be applied from the environment this work was done in (no network path to the Neon host).
